...
for (let idx = 1; idx < recipes.length; idx++) {
    item_container.insertAdjacentHTML('beforeend',
     `  <div class="column" id="${recipes[idx].id}" onClick="showDetailItem(this.id);">
             <div class="item-content">
             <img src="${recipes[idx].img}">
             <span class="item-desc">${recipes[idx].name}</span>
             </div>  
        </div>`);       
    } 
}
